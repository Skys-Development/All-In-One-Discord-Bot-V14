...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (config.dnsLocked && interaction.user.id !== config.OWNER_ID) {
      return interaction.reply({ content: '🔒 DNS creation is currently locked.', ephemeral: true });
    }

    const domains = config.domains || [];
    if (domains.length === 0) {
      return interaction.reply({ content: '❌ No domains configured.', ephemeral: true });
    }

    const domainName = interaction.options.getString('domain');
    const domainConfig = domains.find(d => d.name === domainName);
    if (!domainConfig) {
